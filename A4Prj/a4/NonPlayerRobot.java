...
////		t.rota
////		t.rotate((float)Math.toRadians(this.getHeading()), 0.0f , 0.0f);
//		t.setRotation((float)Math.toRadians(this.getHeading()), x, y);
//		g.setTransform(t);
		
		// Move the origin of the coordinate system to the center of the object
        t.translate((float)this.getLocation().getX() + this.getSize() / 2, (float)this.getLocation().getY() + this.getSize() / 2);

        // Rotate the object by the desired angle in radians
        t.rotate((float)Math.toRadians(((Movable)this).getHeading()), 0.0f , 0.0f);

        // Move the origin of the coordinate system back to the top-left corner of the object
        t.translate(-(float)this.getLocation().getX() + this.getSize() / 2, -(float)this.getLocation().getX() + this.getSize() / 2);
